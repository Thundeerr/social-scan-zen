...
VALUES (true, 15000, 85)
ON CONFLICT (id) DO UPDATE SET monthly_cap = 15000, updated_at = now();
...
REVOKE ALL ON FUNCTION public.provider_budget_usage(timestamptz, timestamptz) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.provider_budget_usage(timestamptz, timestamptz) TO authenticated, service_role;
